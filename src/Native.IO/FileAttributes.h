...
namespace Native
{
	namespace IO
	{
        ...
        enum class FileAttributes
        {
            ...
            /// Windows sets this attribute whenever the file is modified, and backup software  
            /// should clear it when processing the file during incremental backup.
            /// </summary>
            Archive = 32,
            ...
            /// This attribute is valid only if it is used alone. Normal is supported on  
            ...
            /// associated with a file or a directory. ReparsePoint is supported on Windows,  
            /// Linux, and macOS.
            /// </summary>
            ReparsePoint = 1024,
        ...
        constexpr FileAttributes operator &(const FileAttributes lhs, const FileAttributes rhs) {
            using T = std::underlying_type_t<FileAttributes>;
            return static_cast<FileAttributes>(static_cast<T>(lhs) & static_cast<T>(rhs));
        }
        ...
        constexpr FileAttributes operator~(const FileAttributes f) {
            using T = std::underlying_type_t<FileAttributes>;
            return static_cast<FileAttributes>(~static_cast<T>(f));
        }
	}
}